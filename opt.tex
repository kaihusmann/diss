...
\begin{table}[ht]
	\centering
		...
		\begin{tabular}{ccc}
			...
			\hline
			1 & \begin{tabular}{c}determined\\standard treatment\end{tabular} & no restrictions \\ 
			2 & optimized treatment & no restrictions \\ 
			3 & optimized treatment & \begin{tabular}{c}annual minimum delivery\\amounts for low-valued deciduous\\wood from thinning activities\end{tabular}
			\\
			\hline
		\end{tabular}
\end{table}
...
\begin{table}[]
	\centering
	\caption{Stand volumes of the five forest stands from the first exemplary forest enterprise at the end of the simulated scenarios (year 2031). The stand volume of the fifth stand in composed of two species... distinguished by tree species}
	...
	\begin{tabular}{ccccc}
		...
		3                      &  & 356                    & 567.8                    & 912.3                   \\
		4                      &  & 392                    & 567.8                    & 912.3                   \\
		5                      &  & 373                    & 567.8                    & 912.3                   \\ \cline{1-1} \cline{3-5} 
	\end{tabular}
\end{table}

As the first exemplary enterprise was composed of five stands only, the development of each stand was recorded in detail. Table \ref{tab:discussion:struct:opt:application:tab1} shows the stand volumes at the beginning, Table \ref{tab:discussion:struct:opt:application:vol_after} at the end of the simulations. The stand volumes of the standard treatment simulation (scenario 1) built the 
predefined target volumes for the optimization scenarios. The stand volumes of scenario~1 are proxy variables for the implemented sustainability restriction (Subsection \ref{subsec:intro:struct:opt}). The stand volumes increased or stayed constant. This reflects the typical wood usage below the average wood growth in intermediate-ages European beech stands (Figure \ref{fig:hzb:fig4_zuw_nutz}). It was the expectable development under the given species and age mixture.

The si\-mu\-la\-tion-op\-ti\-mi\-za\-tion software was programmed such that the sustainability limitations base, for simplification reasons, on the stand volumes of scenario 1. The stand volumes in the last simulated year of the second scenario are therefore expected to be similar. Deviations from the stand volumes of the standard scenario are only allowed in very narrow intervals. Table \ref{tab:discussion:struct:opt:application:vol_after} reveals the fulfillment of the implemented sustainability restriction. It showed up that each stand volume at the end of scenario 1 differed only slightly from the stand volumes at the end of the standard simulations.
